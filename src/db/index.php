...
$uniqueFirstLetters = array_map( function($airport) { 
    return $airport['name']; 
}, $stmt->fetchAll() );
...
$airportsLength = $stmt->fetchAll()[0][0];
...
$limit = '';
if (isset($_GET['page'])) {
    $offset = ($_GET['page'] - 1) * $limitPerPage;
    ...
    <nav aria-label="Navigation">
        <ul class="pagination justify-content-center">
            <?php for ($i = 1; $i <= ceil($airportsLength / $limitPerPage); $i++): ?>         
                <li class="page-item <?= (isset($_GET['page']) && $i == $_GET['page']) || 
